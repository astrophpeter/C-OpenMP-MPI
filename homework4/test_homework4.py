...
def plot_example_serial_and_parallel(Nt=100):
    ...
    Nx = N // comm.size  # number x,u-points in each process's chunk
    dx = 1.0/(N+1)       # step size in x-domain
    dt = 0.51*dx**2       # must be <= 0.5*dx**2 or else F.E. is unstable

    u_parallel = example_parallel_solution(dx, N, dt, Nt, comm)
    u_parallel2 =example_parallel_solution(dx, N, dt, 200, comm)
    u_parallel3 = example_parallel_solution(dx, N, dt, 300, comm)
    u_parallel4 =example_parallel_solution(dx, N, dt, 400, comm)
    ...
    if (comm.rank == 0):
        ...
        u_serial2 = heat_serial(u_initial, dx, N, dt, 200)
        u_serial3 = heat_serial(u_initial, dx, N, dt, 300)
        u_serial4 = heat_serial(u_initial, dx, N, dt, 400)

        # plot results
        plt.clf()
        plt.plot(u_initial,'k--', linewidth=3, label='Initial Condition')
        
        plt.plot(u_serial, 'r--', linewidth=3, label='S:Nt=100')
        plt.plot(u_serial2, 'b--', linewidth=3, label='S:Nt=200')
        plt.plot(u_serial3, 'g--', linewidth=3, label='S:Nt=300')
        plt.plot(u_serial4, 'y--', linewidth=3, label='S:Nt=400')
       
        plt.plot(u_parallel, 'r-', label='P: Nt=100')
        plt.plot(u_parallel2, 'b-', label='P: Nt=200')
        plt.plot(u_parallel3, 'g-', label='P: Nt=300')
        plt.plot(u_parallel4, 'y-', label='P: Nt=400')
        ...
        plt.title('Parallel Heat Equation Solution $dx=%.2e \; dt=%.2e \nfor Various Nt P (Parallel), S (serial)'%(dx,dt),
                  fontsize=14)
